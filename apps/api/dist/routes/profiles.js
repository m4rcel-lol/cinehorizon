...
profileRouter.post('/', asyncHandler(async (req, res) => {
    ...
    if (count >= 5)
        throw new AppError(400, 'PROFILE_LIMIT', 'Maximum of 5 profiles reached');
...
profileRouter.patch('/:id', asyncHandler(async (req, res) => {
    ...
    if (body.avatarIndex !== undefined)
        data.avatarIndex = body.avatarIndex;
...
profileRouter.delete('/:id', asyncHandler(async (req, res) => {
    ...
    const count = await prisma.profile.count({ where: { userId: req.auth.userId } });
    if (count <= 1)
        throw new AppError(400, 'LAST_PROFILE', 'Cannot delete the last profile');
