...
adminRouter.get('/users', asyncHandler(async (req, res) => {
    ...
    const where = query.q ? { OR: [{ email: { contains: query.q, mode: 'insensitive' } }, { displayName: { contains: query.q, mode: 'insensitive' } }] } : {};
...
adminRouter.delete('/users/:id', asyncHandler(async (req, res) => {
    ...
    if (id === req.auth.userId)
        throw new AppError(400, 'SELF_DELETE_FORBIDDEN', 'You cannot delete your own admin account');
...
adminRouter.get('/content', asyncHandler(async (req, res) => {
    ...
    const where = { ...(query.status ? { status: query.status } : {}), ...(query.type ? { type: query.type } : {}), ...(query.q ? { title: { contains: query.q, mode: 'insensitive' } } : {}) };
...
adminRouter.patch('/content/:id', asyncHandler(async (req, res) => {
    ...
    if (body.backdropUrl !== undefined)
        data.backdropUrl = body.backdropUrl;
    if (body.posterUrl !== undefined)
        data.posterUrl = body.posterUrl;
    if (body.logoUrl !== undefined)
        data.logoUrl = body.logoUrl;
    if (body.trailerUrl !== undefined)
        data.trailerUrl = body.trailerUrl;
    if (body.isFeatured !== undefined)
        data.isFeatured = body.isFeatured;
    if (body.isOriginal !== undefined)
        data.isOriginal = body.isOriginal;
    if (body.isTrending !== undefined)
        data.isTrending = body.isTrending;
    if (body.isTopTen !== undefined)
        data.isTopTen = body.isTopTen;
    if (body.topTenRank !== undefined)
        data.topTenRank = body.topTenRank;
    if (body.status !== undefined)
        data.status = body.status;
    if (body.maturityTags !== undefined)
        data.maturityTags = body.maturityTags;
    if (body.genreIds !== undefined)
        data.genres = { set: body.genreIds.map((genreId) => ({ id: genreId })) };
...
async function receiveVideoUpload(req, target) {
    ...
    return new Promise((resolve, reject) => {
        ...
        busboy.on('file', (_field, file, info) => {
            ...
            file.on('data', (chunk) => {
                if (Buffer.concat(chunks).length < 12)
                    chunks.push(chunk.subarray(0, Math.max(0, 12 - Buffer.concat(chunks).length)));
        ...
        busboy.on('finish', () => { if (!resolved)
            reject(new AppError(400, 'NO_FILE', 'No video file uploaded')); });
...
adminRouter.post('/content/:id/seasons/:season/episodes/:ep/video', asyncHandler(async (req, res) => {
    ...
    if (!episode)
        throw new AppError(404, 'EPISODE_NOT_FOUND', 'Episode not found');
...
adminRouter.get('/uploads/status/:jobId', asyncHandler(async (req, res) => {
    ...
    if (!job)
        throw new AppError(404, 'UPLOAD_NOT_FOUND', 'Upload job not found');
    res.json({ job });
}));
