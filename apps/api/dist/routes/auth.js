...
async function issueAuth(user, req, res) {
...
authRouter.post('/refresh', asyncHandler(async (req, res) => {
    const raw = req.cookies?.ch_refresh;
    if (!raw)
    ...
    if (!session || session.expiresAt < new Date())
        throw new AppError(401, 'REFRESH_EXPIRED', 'Refresh token is invalid or expired');
...
authRouter.post('/logout', asyncHandler(async (req, res) => {
    const raw = req.cookies?.ch_refresh;
    if (raw)
        await prisma.session.deleteMany({ where: { token: sha256(raw) } });
...
authRouter.post('/verify-email', asyncHandler(async (req, res) => {
    ...
    if (!record || record.usedAt || record.expiresAt < new Date())
        throw new AppError(400, 'BAD_VERIFICATION_TOKEN', 'Verification token is invalid or expired');
