...
export default function App(){ const setAuth = useAdminStore((s)=>s.setAuth); useEffect(()=>{ void api<any>('/auth/refresh',{method:'POST'}).then((d)=>{ if(d.user.role==='ADMIN') setAuth(d.user,d.accessToken); }).catch(()=>undefined);},[setAuth]); return <Routes><Route path="/login" element={<Login/>}/><Route path="/" element={<RequireAdmin><Shell><Dashboard/></Shell></RequireAdmin>}/><Route path="/content" element={<RequireAdmin><Shell><Content/></Shell></RequireAdmin>}/><Route path="/games" element={<RequireAdmin><Shell><Downloads category="GAME"/></Shell></RequireAdmin>}/><Route path="/software" element={<RequireAdmin><Shell><Downloads category="SOFTWARE"/></Shell></RequireAdmin>}/><Route path="/users" element={<RequireAdmin><Shell><Users/></Shell></RequireAdmin>}/><Route path="/uploads" element={<RequireAdmin><Shell><Uploads/></Shell></RequireAdmin>}/><Route path="/settings" element={<RequireAdmin><Shell><Settings/></Shell></RequireAdmin>}/></Routes>; }
